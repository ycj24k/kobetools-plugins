<template>
  <a-form class="form_box" ref="XGSSFormRef" layout="vertical" hide-label :model="XGSSForm" @submit="XGSSFormSubmit">
      ...
      <a-grid-item :span="18" class="form_right">
        <div class="flex_box form_item form_item_radio">
          ...
          <a-form-item no-style field="support" :rules="[{ required: true, message: localeGet('message1') }]" :validate-trigger="['change', 'blur']">
            <a-radio-group v-model="XGSSForm.support" :options="translatedXgssSupportOptions"></a-radio-group>
          </a-form-item>
        </div>
        <div class="form_item">
          <a-grid :col-gap="20">
            <a-grid-item :span="12">
              <div class="flex_box form_item_radio">
                ...
                <a-form-item no-style field="depth" :rules="[{ required: true, message: localeGet('message2') }]" :validate-trigger="['change', 'blur']">
                  <a-radio-group v-model="XGSSForm.depth" :options="translatedDepthOptions"></a-radio-group>
                </a-form-item>
              </div>
            </a-grid-item>
            <a-grid-item :offset="6" :span="6" class="flex_box form_option">
              <div class="form_label">{{ localeGet('label3') }}</div>
              <a-form-item no-style field="reserve">
                <a-space :size="20">
                  <a-switch v-model="XGSSForm.reserve" :checked-value="1" :unchecked-value="0" />
                </a-space>
              </a-form-item>
            </a-grid-item>
          </a-grid>
          ...
          <a-grid :col-gap="20" :row-gap="10" class="form_content">
            <a-grid-item :span="14" class="flex_box form_option">
              ...
              <a-form-item no-style field="lengthFilter">
                <a-space :size="20">
                  <a-switch v-model="XGSSForm.lengthFilter" :checked-value="1" :unchecked-value="0" />
                  <template v-if="XGSSForm.lengthFilter === 1">
                    <a-space :size="20">
                      <span>{{ localeGet('label6') }}</span>
                      <a-select v-model="XGSSForm.lengthFilterVal.min" :options="translatedLengthMinOptions" :style="{ width: '140px' }" :placeholder="localeGet('placeholder3')"></a-select>
                      <span>-</span>
                      <span>{{ localeGet('label7') }}</span>
                      <a-select v-model="XGSSForm.lengthFilterVal.max" :options="translatedLengthMaxOptions" :style="{ width: '140px' }" :placeholder="localeGet('placeholder4')"></a-select>
                    </a-space>
                  </template>
                </a-space>
              </a-form-item>
            ...
            <a-grid-item :span="10" class="flex_box form_option">
              ...
              <a-form-item no-style field="sensitiveFilter">
                <a-space :size="20">
                  <a-switch v-model="XGSSForm.sensitiveFilter" :checked-value="1" :unchecked-value="0" />
                  <template v-if="XGSSForm.sensitiveFilter === 1">
                    <a-select v-model="XGSSForm.sensitiveFilterVal" :options="translatedCustomOptions" :style="{ width: '220px' }" allow-search :placeholder="localeGet('placeholder2')"></a-select>
                  </template>
                </a-space>
              </a-form-item>
            </a-grid-item>
            ...
            <a-grid-item :span="12" class="flex_box form_content_item">
              ...
              <div class="flex_box form_content_top">
                ...
                <a-form-item no-style field="include">
                  <a-radio-group v-model="XGSSForm.include" :options="translatedIncludeOptions"></a-radio-group>
                </a-form-item>
              </div>
            </a-grid-item>
            <a-grid-item :span="12" class="form_content_item">
              <div class="flex_box form_content_top">
                ...
                <a-form-item no-style field="exclude">
                  <a-radio-group v-model="XGSSForm.exclude" :options="translatedExcludeOptions"></a-radio-group>
                </a-form-item>
              </div>
            </a-grid-item>
          </a-grid>
          <a-grid :col-gap="20" :row-gap="10" class="form_content">
            <a-grid-item :span="12" class="flex_box form_content_item">
              <div class="form_content_input">
                <a-textarea v-model="XGSSForm.includeKeyword" class="form_area" :placeholder="localeGet('placeholder5')" allow-clear />
              </div>
            </a-grid-item>
            <a-grid-item :span="12" class="form_content_item">
              <div class="form_content_input">
                <a-textarea v-model="XGSSForm.excludeKeyword" class="form_area" :placeholder="localeGet('placeholder5')" allow-clear />
              </div>
            </a-grid-item>
          </a-grid>
        </div>
...
// 翻译选项
const translatedXgssSupportOptions = translateOptions(xgssSupportOptions);
...
// 关键词挖掘
// const xgssSupportOptions = ref([]);
const XGSSFormRef = ref(null);
const keyword = ref('');
const XGSSForm = ref({ ...XGSSFormDefault });
...
//     xgssSupportOptions.value = res.data;
//   } catch (error) {}
// };
// getSupportList();
// 关键词挖掘提交
const XGSSFormSubmit = async ({ errors, values }) => {
  ...
  if (!errors) {
    ...
    try {
      ...
      if (XGSSForm.value.reserve) {
        XGSSForm.value.reserveKeyword = keyword.value;
      }
      // 字符长度过滤
      if (XGSSForm.value.lengthFilter) {
        const { min, max } = XGSSForm.value.lengthFilterVal;
        if (min > 0 || max > 0) {
          XGSSForm.value.keyword = XGSSForm.value.keyword.filter((item) => (min === 0 && max > 0 && item.length <= max) || (min > 0 && max === 0 && item.length >= min) || (min > 0 && max > 0 && item.length >= min && item.length <= max));
        }
      }
      // 包含关键词
      if (XGSSForm.value.includeKeyword.length) {
        const includeKeyword = XGSSForm.value.includeKeyword.split('\n');
        if (XGSSForm.value.include) {
          XGSSForm.value.keyword = XGSSForm.value.keyword.filter((item) => includeKeyword.some((char) => item.includes(char)));
        } else {
          XGSSForm.value.keyword = XGSSForm.value.keyword.filter((item) => includeKeyword.every((char) => item.includes(char)));
        }
      }
      // 不包含关键词
      if (XGSSForm.value.excludeKeyword.length) {
        const excludeKeyword = XGSSForm.value.excludeKeyword.split('\n');
        if (XGSSForm.value.exclude) {
          XGSSForm.value.keyword = XGSSForm.value.keyword.filter((item) => !excludeKeyword.some((char) => item.includes(char)));
        } else {
          XGSSForm.value.keyword = XGSSForm.value.keyword.filter((item) => !excludeKeyword.every((char) => item.includes(char)));
        }
      }
      console.log(XGSSForm.value.keyword);
      if (XGSSForm.value.keyword.length === 0) {
        Message.warning(localeGet('message4'));
        return;
      }
      keywordTaskAdd(XGSSForm.value)
        .then((res) => {
          Message.success(localeGet('message5'));
          XGSSForm.value = { ...XGSSFormDefault };
          keyword.value = '';
