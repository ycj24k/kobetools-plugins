    ...
    <a-grid class="form_main">
      ...
      <a-grid-item :span="24" class="form_right">
          ...
          <a-grid  :col-gap="20" :row-gap="10" class="form_content">
            <a-grid-item :span="12" class="flex_box">
              ...
              <a-form-item no-style field="support" :rules="[{ required: true, message: localeGet('message1') }]" :validate-trigger="['change', 'blur']">
                <a-radio-group v-model="jingduiForm.support" :options="supportOptions">
                  <!-- <template #label="{ data }">
            ...
            <a-grid-item :span="12" class="flex_box" v-if="jingduiForm.support !== 1">
              ...
              <a-form-item no-style field="engine" :rules="[{ required: true, message: '请选择搜索引擎' }]" :validate-trigger="['change', 'blur']">
                <a-checkbox-group v-model="jingduiForm.engine" :options="engineOptions">
                  <!-- <template #label="{ data }">
                    <span>{{ localeGet(data?.label) }}</span>
                  </template>
                  <template #option="{ data }">
        ...
        <div class="flex_box form_item">
          <a-form-item no-style field="keyword">
            <a-textarea v-model="keyword" class="form_area" placeholder="请输入需要提取关键词的竞争对手网址链接，一行一条url链接，需带https或http的网站链接地址" allow-clear />
          </a-form-item>
        </div>
        <div class="flex_box form_item">
          ...
          <a-grid :col-gap="20" :row-gap="10" class="form_content">
            <a-grid-item :span="12" class="flex_box form_option">
              ...
              <a-form-item no-style field="lengthFilter">
                <a-space :size="20">
                  <a-switch v-model="jingduiForm.lengthFilter" :checked-value="1" :unchecked-value="0" />
                  <template v-if="jingduiForm.lengthFilter === 1">
                    <a-space :size="20">
                      <span>{{ localeGet('label6') }}</span>
                      <a-select v-model="jingduiForm.lengthFilterVal.min" :options="lengthMinOptions" :style="{ width: '140px' }" :placeholder="localeGet('placeholder3')">
                        <template #label="{ data }">
                          <span>{{ localeGet(data?.label) }}</span>
                        </template>
                        <template #option="{ data }">
                      ...
                      <a-select v-model="jingduiForm.lengthFilterVal.max" :options="lengthMaxOptions" :style="{ width: '140px' }" :placeholder="localeGet('placeholder4')">
                        <template #label="{ data }">
                          <span>{{ localeGet(data?.label) }}</span>
                        </template>
                        <template #option="{ data }">
            ...
            <a-grid-item :span="12" class="flex_box form_option">
              ...
              <a-form-item no-style field="sensitiveFilter">
                <a-space :size="20">
                  <a-switch v-model="jingduiForm.sensitiveFilter" :checked-value="1" :unchecked-value="0" />
                  <template v-if="jingduiForm.sensitiveFilter === 1">
                    <a-radio-group v-model="jingduiForm.sensitiveFilterVal" :options="sensitiveOptions">
                      <template #label="{ data }">
                        <span>{{ localeGet(data?.label) }}</span>
                      </template>
                      <template #option="{ data }">
                  ...
                  <template v-if="jingduiForm.sensitiveFilter === 1 && jingduiForm.sensitiveFilterVal === 2">
                    <a-select v-model="jingduiForm.sensitiveCustom" :options="customOptions" :style="{ width: '220px' }" allow-search :placeholder="localeGet('placeholder2')">
                      <template #label="{ data }">
                        <span>{{ localeGet(data?.label) }}</span>
                      </template>
                      <template #option="{ data }">
...
import { jingduiFormDefault, includeOptions, excludeOptions, supportOptions, engineOptions, sensitiveOptions, lengthMinOptions, lengthMaxOptions, customOptions } from '../../utils/config';
