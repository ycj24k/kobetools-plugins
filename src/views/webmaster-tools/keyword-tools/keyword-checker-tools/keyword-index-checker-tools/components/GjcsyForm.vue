    ...
    <div style="display: flex; flex-direction: column; height: 100%;">
        ...
        <div style="display: flex; gap: 12px;">
            <div style="display: flex; gap: 12px; width: 300px;">
                ...
                <a-checkbox-group v-model="queryParam.searchEngines">
                    <a-checkbox v-for="searchEngine in searchEngines" :value="searchEngine.code">{{searchEngine.name}}</a-checkbox>
                </a-checkbox-group>
            </div>
        </div>
        ...
        <div style="flex: 1;">
            <XTextarea v-model="queryParam.keywords"
                       placeholder="请输入需要查询索引的关键词，一行一个，一次最多提交100个"/>
        </div>
...
function queryTableData() {
    ...
    if (queryParam.searchEngines.length === 0 || queryParam.keywords.trim().length === 0) {
...
// 全选
function allSelect() {
    queryParam.searchEngines = searchEngines.map(searchEngine => searchEngine.code);
}

// 反选
function revoltSelect() {
    queryParam.searchEngines = searchEngines
        .filter(searchEngine => !queryParam.searchEngines.includes(searchEngine.code))
        .map(searchEngine => searchEngine.code);
}
