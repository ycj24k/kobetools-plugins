...
watch(() => props.locales, (newVal) => {
    ...
    columns.value = [
        { title: localeGet('linkColumns.label1'), dataIndex: 'serialNumber', sortable: { sortDirections: ['ascend', 'descend'] }, width: 100 },
        { title: localeGet('linkColumns.label2'), dataIndex: 'domain', sortable: { sortDirections: ['ascend', 'descend'] }, width: 150 },
        { title: localeGet('linkColumns.label3'), dataIndex: 'title', width: 200 },
        { title: localeGet('linkColumns.label4'), dataIndex: 'keywords', sortable: { sortDirections: ['ascend', 'descend'] }, width: 150 },
        { title: localeGet('linkColumns.label5'), dataIndex: 'description', width: 200 },
    ];
}, { immediate: true });
...
function exportRecordKeepingDomains() {
    if (xTable.value.table.tableCurrData.length === 0) {
        showErrorNotification(localeGet('message3') || localeGet('message2'));
        return;
    }
