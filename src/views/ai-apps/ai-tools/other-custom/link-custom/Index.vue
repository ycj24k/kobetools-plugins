<template>
  <div class="container">
    <div class="container_box">
      ...
      <div class="table_box">
        <a-table column-resizable :bordered="{ cell: true }" :loading="tableLoading" :columns="innerlinkTableColumns" :data="tableData" :row-selection="rowSelection" v-model:selectedKeys="selectedKeys" :pagination="pagination" @page-size-change="handlePageSizeChange" :scroll="{ x: '100%', y: 500 }">
...
const getList = async () => {
  ...
  try {
    const res = await innerlinkList(queryForm.value);
...
const addForm = ref({ ...innerlinkFormDefault });
const addVisible = ref(false);
const handleAdd = () => {
  addForm.value = { ...innerlinkFormDefault };
...
const handleDetail = (rowIndex) => {
  ...
  router.push({ path: '/ai-apps/ai-tools/other-custom/link-detail', query: { tid: tableData.value[rowIndex].id } });
};
// 编辑
...
<script>
export default {
  name: 'OtherInner', // If you want the include property of keep-alive to take effect, you must name the component
};
</script>
