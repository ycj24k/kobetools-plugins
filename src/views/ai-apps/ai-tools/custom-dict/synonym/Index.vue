<template>
  <div class="container">
    <div class="container_box">
      ...
      <div class="table_box">
        <a-table column-resizable :bordered="{ cell: true }" :loading="tableLoading" :columns="synonymTableColumns" :data="tableData" :row-selection="rowSelection" v-model:selectedKeys="selectedKeys" :pagination="pagination" @page-size-change="handlePageSizeChange" :scroll="{ x: '100%', y: 500 }">
          ...
          <template #replace="{ rowIndex }">
            <a-input @change="handleSaveChange(rowIndex, 'replace')" v-model="tableData[rowIndex].replace" />
          </template>
...
import { synonymTableColumns } from '../utils/config';
...
const getGroupList = async () => {
  try  {
    const res = await wordGroupList({ type: 2 });
...
<script>
export default {
  name: 'CustomSynonym', // If you want the include property of keep-alive to take effect, you must name the component
};
</script>
