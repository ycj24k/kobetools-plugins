<template>
  <div class="container">
    <div class="container_box x-tabs-fill">
        ...
        <a-grid :col-gap="24" :row-gap="12">
          ...
          <a-grid-item :span="4" class="flex_box">
            <a-form-item field="type">
              <a-select v-model="queryForm.type" :options="typeOptions" allow-search :placeholder="localeGet('placeholder2')">
                <template #label="{ data }">
      ...
      <div class="table_box">
        <a-table column-resizable :bordered="{ cell: true }" :loading="tableLoading" :columns="groupTableColumns" :data="tableData" :row-selection="rowSelection" v-model:selectedKeys="selectedKeys" :pagination="pagination" @page-size-change="handlePageSizeChange" :scroll="{ x: '100%', y: 500 }">
          ...
          <template #actions="{ rowIndex }">
            <div class="flex_box flex_row_between table_btns">
              <a-popconfirm position="left" :content="localeGet('message1')" @ok="handleExport(rowIndex)">
                <a-button size="mini" class="form_btn5">{{ localeGet('btn4') }}</a-button>
              </a-popconfirm>
            </div>
          </template>
        </a-table>
...
import { wordGroupList, wordGroupSave } from '@/api/apps/tools/custom';
import { typeOptions, groupTableColumns } from '../utils/config';
...
const getList = async () => {
  ...
  try {
    const res = await wordGroupList(queryForm.value);
...
<script>
export default {
  name: 'CustomGroup', // If you want the include property of keep-alive to take effect, you must name the component
};
</script>
